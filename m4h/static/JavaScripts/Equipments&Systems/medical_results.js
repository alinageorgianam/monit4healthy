...
function fetchPatientData() {
    ...
    if (patientId) {
        fetch(`/get_patient_data/${patientId}`)
            ...
            .then(data => {
                data.blackbox_measurements = undefined;
                data.blackbox_measurements = undefined;
                data.gaitband_measurements = undefined;
                data.gaitband_measurements = undefined;
                data.emg_measurements = undefined;
                data.emg_measurements = undefined;
                ...
                if (data.blackbox_measurements.length > 0) {
                    ...
                    data.blackbox_measurements.forEach(measurement => {
                        measurement.ekg = undefined;
                        measurement.pulse = undefined;
                        measurement.spo2 = undefined;
                        measurement.body_temp = undefined;
                        measurement.co2 = undefined;
                        measurement.alcohol = undefined;
                ...
                if (data.gaitband_measurements.length > 0) {
                    ...
                    data.gaitband_measurements.forEach(measurement => {
                        measurement.fall_detected = undefined;
                        const row = document.createElement('tr');
                        row.innerHTML = `
                            <td>${measurement.timestamp}</td>
                ...
                if (data.emg_measurements.length > 0) {
                    ...
                    data.emg_measurements.forEach(measurement => {
                        measurement.muscle_activity = undefined;
                        const row = document.createElement('tr');
                        row.innerHTML = `
                            <td>${measurement.timestamp}</td>
